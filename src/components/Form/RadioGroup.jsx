...
const RadioGroup = ({ label, options = [], setFieldValue, placeholder, success, ...props }) => {
  const [id] = useState(() => uniqueId('radio-group-id-'));
      ...
      <div className="field--horizontal">
        {
          options.map(({ value, text }) => {
            ...
            return (
              <div className={`radio-set ${is_selected_class}`} key={`${id}-${text}-${value}`}>
                <input
                  type="radio"
                  id={radio_item_id}
                  name={field.name}
